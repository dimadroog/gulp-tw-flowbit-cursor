...
3. Запусти проверочные артефакты:
   ...
   - результат `pre-final-self-check` (`ready_for_finalize`)
   - результат `finalize-layout-task` уже с `overall_status: pass` по сводной матрице (эта команда **не** входит в ту матрицу; `validate-all-directives` запускай только после успешного finalize)
