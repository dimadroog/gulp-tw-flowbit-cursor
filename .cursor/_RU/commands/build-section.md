...
7. Оптимизируй медиа и структуру секции под скорость загрузки (корректные форматы, исходные размеры близки к отображаемым, без лишних оберток).
8. Используй `picture`/`srcset` для адаптивных изображений и задавай корректный `alt` (или `alt=""` для декоративных изображений).
9. Держи atomic-классы в разметке в каноническом порядке сортировки `prettier-plugin-tailwindcss`.
10. Если добавлен custom CSS, соблюдай порядок свойств от layout-критичных к локально-декоративным.
11. Для шрифтов в секции проверяй, что используются только нужные семейства/начертания и в кастомных объявлениях сохранен `font-display: swap`.
12. Выполни контент-стресс-проверку: добавь более длинные текстовые варианты и замени изображения на файлы с другими размерами/пропорциями.
13. Проверь focus и keyboard behavior для интерактивных контролов.
14. Перед завершением секции запусти `performance-checklist`.
...
- Доставка изображений адаптивна (`picture`/`srcset`), а все `img` имеют корректно заданный `alt`.
