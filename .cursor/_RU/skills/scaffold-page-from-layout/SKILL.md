...
8. Для каждой секции задай адаптивную стратегию изображений через `picture`/`srcset`, где это уместно.
9. Явно задай стратегию загрузки изображений (`lazy` или `eager`) и держи исходные размеры близкими к отображаемым.
10. Для каждого `img` обеспечь корректный `alt` (осмысленный текст или пустой `alt=""` для декоративных изображений).
11. Проведи быструю проверку устойчивости на длинном тексте и альтернативных пропорциях изображений.
...
- Базовая верстка устойчива к изменению длины контента и пропорций изображений.
