...
8. Запланируй слоты изображений по [`rules/image-delivery-and-optimization.RULE.md`](../../rules/image-delivery-and-optimization.RULE.md) (`loading`, `picture`/`srcset`, intrinsic ≤ 2× rendered, форматы).
9. Для каждого `img` обеспечь корректный `alt` по [`rules/accessibility-and-w3c.RULE.md`](../../rules/accessibility-and-w3c.RULE.md).
